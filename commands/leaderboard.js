...
module.exports = {
	...
	async execute(client, message, args) {
		...
		if (operator === 'money') {
			...
			var embed = new Discord.MessageEmbed().setTitle('Dogie Coin Leaderboard')
		} else {
			var result = await Levels.find({ serverId: message.guild.id })
				.sort([['xp', 'descending']])
				.catch((err) => console.log(err))
			var embed = new Discord.MessageEmbed().setTitle('Dogie Level Leaderboard')
		}
		if (result.length === 0) {
			embed.setColor('RED')
			...
			for (let i = 0; i < length; i++) {
				...
				if (operator === 'money') {
					...
					if (!levelSchema) await embed.addField(`${i + 1}: ${user.username}`, `**Coins:** ${result[i].money}\, **Level: N/A**`)
					else await embed.addField(`${i + 1}: ${user.username}`, `**Coins:** ${result[i].money}\, **Level:** ${Math.floor(Math.cbrt(levelSchema.xp / 1.25))}\, **XP:** ${levelSchema.xp}`)
					...
					if (!moneySchema) await embed.addField(`${i + 1}: ${user.username}`, `**Level:** ${Math.floor(Math.cbrt(result[i].xp / 1.25))}\, **XP:** ${result[i].xp}\, **Coins: N/A**`)
					...
					else await embed.addField(`${i + 1}: ${user.username}`, `**Level:** ${Math.floor(Math.cbrt(result[i].xp / 1.25))}\, **XP:** ${result[i].xp}\, **Coins:** ${moneySchema.money}`)
				}
			}
		}
