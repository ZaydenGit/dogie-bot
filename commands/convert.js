...
module.exports = {
	...
	async execute(client, message, args) {
		...
		if (!moneySchema) moneySchema = await new Money({ userID: message.author.id, serverID: message.guild.id, moneySchema: 0 }).save().catch((e) => console.log(e))
		...
		if (level > Math.floor(Math.cbrt((levelSchema.xp - arg) / 1.25))) return message.reply(`You have converted ${arg} Dogie Coins into XP and levelled up to ${level}! (${levelSchema.xp - 1.25 * level ** 3}/${1.25 * (level + 1) ** 3 - 1.25 * level ** 3}). You now have ${money.money} Dogie Coins`)
